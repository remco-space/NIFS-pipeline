...
#	   Jul-07  R.Blum      - correct for black body slope (fl_fluxcal)
#          Jan-08  R. Blum     - bug fix for loop over multiple input files.
#                                kludge and fl_flux required generation of
#                                cal file to be repeated for each step in 
#                                the main loop.
#
# Still to do:
...
bool fl_fluxcal  {yes,       prompt="Flux calbibrate (multiply by black body)?"}
real bbtemp      {10000.,    prompt="Temperature of black body for flux correction"}
...
int  l_lag, l_dispaxis,l_length
real l_threshold, l_shift, l_scale, l_dshift, l_dscale,l_wstart,l_wend,l_delta
...
char tmpcalscale, tmpcalscalevar, tmpcalscaledq, tmpbb, tmpbb1
...
scanfile1 = tmpinimages
scanfile2 = tmpoutimages
# Run telluric. If dispaxis=1, spectrum runs along rows. Main loop.
shifts = "0"
scales = "0"
...
# Normalize calibration spectrum.
  imdelete(tmpcalnorm,verify-,>&"dev$null")
  for(version=1;version<=nversion;version=version+1) {
    ...
    hedit(l_calspec//sci,"AIRMASS",calairmass,add+,addonly-,delete-,verify-,show-,update+)
    if (version == 1) {
  ...
  hedit(imgin//"["//l_sci_ext//"]","AIRMASS",real(keypar.value),add+,addonly-,
  ...
  if (twod) {
    printlog("NFTELLURIC - Calling NFEXTRACT to extract 1D spectrum from Input.",
      l_logfile,l_verbose)
#    nfextract(imgin,outspectra=tmpsci1d,outprefix="",fl_inter=l_fl_inter,fl_skysub-, dispaxis=l_dispaxis, logfile=l_logfile,verbose-)
#  had to edit this to remove fl_skysub ACS 2008
    nfextract(imgin,outspectra=tmpsci1d,outprefix="",fl_inter=l_fl_inter,dispaxis=l_dispaxis, logfile=l_logfile,verbose-)
  ...
  tmpbb 	     = mktemp("tmp$nfs")
  tmpbb1 	     = mktemp("tmp$nfs")
  ...
  if (!keypar.found) {
     ...
     hedit(imgi//"["//l_sci_ext//"]","AIRMASS",real(keypar.value),
...
# kludge to write scaled telluric
  imar(imgi//sci,"/",tmpimout,tmpcalnorm//"[sci,overwrite]")
...
# divide telluric by black body

  if (fl_fluxcal) {
    imdelete(tmpbb,verify-,>&"dev$null")
    imdelete(tmpbb1,verify-,>&"dev$null")
    imgets(tmpcalnorm//sci,"CRVAL1")
    l_wstart=real(imgets.value)
    imgets(tmpcalnorm//sci,"CD1_1")
    l_delta=real(imgets.value)
    imgets(tmpcalnorm//sci,"i_naxis1")
    l_length=int(imgets.value)
    l_wend=l_wstart+l_length*l_delta
    artdata.mk1dspec(tmpbb,temperature=bbtemp,wstart=l_wstart,wend=l_wend,ncols=l_length)
    imexpr("a / median(a)",tmpbb1,tmpbb)
    imar(tmpcalnorm//sci,"/",tmpbb1,tmpcalnorm//"[sci,overwrite]")
  }
...
#  imexpr("(a > 0) ? a*b : 1.0",tmpcalscalevar,tmpcalscale,scaler,outtype="real",verbose-)
  ...
  if (nversion > 1) {
    ...
    imexpr("a/b",imgout//sciapp,imgin//sci,basissci,outtype="real",verbose-)
#   Tried editing this line ACS 11/08
#    imexpr("(c/(b*b))+(a*a*d/(b*b*b*b))",imgout//varapp,imgin//sci,basisvar, imgin//var,basisvar,outtype="real",verbose-)
#This works and ignore the error in the telluric spectrum
#    imexpr("a/(b*b)",imgout//varapp,imgin//var,basissci,outtype="real",verbose-)
#But this should be more correct (an edit of the original)
    imexpr("(c/(b*b))+(a*a*d/(b*b*b*b))",imgout//varapp,imgin//sci,basissci, imgin//var,basisvar,outtype="real",verbose-)
  ...
  nhedit (imgout//"[0]", "NFTELCAL", l_calspec, 
    "Calibration file for NFTELLURIC", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELSHF", shiftr,
    "Applied shift (pixels)", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELSCL", scaler,
    "Scale factor for airmass ratio", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELXCO", l_fl_xcorr,
    "Use cross-correlation for shift", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELTWK", l_fl_tweakrms,
    "Tweak to minimize RMS", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELSAM", l_sample,
    "Sample ranges for fits", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELTHR", l_threshold,
    "Threshold for calibration", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELLAG", l_lag,
    "Cross correlation lag (pixels)", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELISH", l_shift,
    "Initial shift (pixels)", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELISC", l_scale,
    "Initial scale factor for airmass ratio", comfile="NULL", after="",
    before="", update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELDSH", l_dshift,
    "Initial shift search step (pixels)", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "NFTELDSC", l_dscale,
    "Initial scale factor search step", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)

  gemdate ()
  nhedit (imgout//"[0]", "NFTELLUR", gemdate.outdate,
    "UT Time stamp for NFTELLURIC", comfile="NULL", after="", before="",
    update+, add+, addonly-, delete-, verify-, show-)
  nhedit (imgout//"[0]", "GEM-TLM", gemdate.outdate,
...
clean:
  ...
  imdelete(tmpbb,ver-, >& "dev$null")
  imdelete(tmpbb1,ver-, >& "dev$null")
