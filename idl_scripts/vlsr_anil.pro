pro helcorr_anil, obs_long, obs_lat, obs_alt, ra2000, dec2000, jd, $
...
r=6378160.0*(0.998327073+0.001676438*cos(2*lat/!RADEG ) $
             -0.00000351*cos(4*lat/!RADEG ) +0.000000008*cos(6*lat/!RADEG )) + obs_alt
...
vbar=vb[0]*cos(dec/!RADEG )*cos(ra/!RADEG )+vb[1]*cos(dec/!RADEG )*sin(ra/!RADEG ) + vb[2]*sin(dec/!RADEG)
vhel=vh[0]*cos(dec/!RADEG )*cos(ra/!RADEG )+vh[1]*cos(dec/!RADEG )*sin(ra/!RADEG ) + vh[2]*sin(dec/!RADEG)
...
function vlsr_anil, ra, dec, date_yr, date_month, date_day, $
                    ut_hrs, ut_mins, ut_sec

;http://www.dark-cosmology.dk/~justyn/iraf/obsdb.dat
;Gemini north
longitude=155.D + 28.142805D0/60.
latitude=19.D + 49.42809D0/60.
altitude = 4213.4D

jd = julday(fix(date_month), fix(date_day), fix(date_yr), $
            fix(ut_hrs), fix(ut_mins), double(ut_sec))

helcorr_anil,longitude,latitude,altitude,ra,dec,jd,corr,hjd
